namespace Test.Shared.Suites
{
    ...
    /// <summary>
    /// Verifies that a structured request with child content chunks in document order across content types.
    /// </summary>
    public static class RequestHierarchySuite
    {
        ...
        public static TestSuiteDescriptor Build()
        {
            return new TestSuiteDescriptor(
                ...
                {
                    new TestCaseDescriptor("RequestHierarchy", "ChildrenChunkedInOrder", "Parent and child content chunk in order and cover all content",
                        executeAsync: async ct =>
                        {
                            ContentRequest root = new ContentRequest
                            {
                                Type = ContentTypeEnum.Text,
                                Text = "Root section introduces the topic with enough words to form a chunk on its own."
                            };
                            ContentRequest childText = new ContentRequest
                            {
                                Type = ContentTypeEnum.Text,
                                Text = "Child section adds supporting detail and also has enough words to chunk cleanly."
                            };
                            ContentRequest childList = new ContentRequest
                            {
                                Type = ContentTypeEnum.List,
                                OrderedList = new List<string> { "first point", "second point", "third point" }
                            };
                            root.Children = new List<ContentRequest> { childText, childList };
                            ...
                            // Position is an ordinal within each parent, so each request's chunks start at 0.
